...
async function scheduleFollowUpCall(lead, delayMs) {
          ...
          setTimeout(async function() {
                      ...
                      try {
                        ...
                        // Skip if lead is no longer eligible
          ...
          try {
           ...
           if (needsFollowUp && currentCallCount < MAX_CALLS_PER_LEAD) {
            ...
            if (lead.phone) {
                          ...
                          console.log(`[retell-callback] Lead ${leadId} needs follow-up (status="${callStatus}", outcome="${outcome}", calls so far=${currentCallCount}/${MAX_CALLS_PER_LEAD}). Scheduling for tomorrow 9PM IST.`);
                          await safeUpdateZohoLead(leadId, {
...
// Runs every day at 9PM IST automatically forever.
// Fetches all leads with AI_Call_Count 1 or 2 and schedules their next call.
// Skips: terminal statuses, no phone, never called (count=0), maxed out (count>=3)
