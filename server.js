...
// Writes free-text/URL/DateTime fields first (always safe),
// then pick-list fields one at a time so a bad value doesn't block others.
async function safeUpdateZohoLead(leadId, fields) {
      // Step 1: Write all non-picklist fields together
  const safeFields = {};
  ...
  // Step 2: Write each picklist field individually so failures are isolated
...
// Guard: Only calls if AI_Last_Call_Status is empty (lead has never been called before)
app.post('/webhook/zoho-lead', async (req, res) => {
      console.log('[zoho-lead] Incoming payload:', JSON.stringify(req.body, null, 2));
        ...
        // Write call initiated status using safe updater
        try {
...
app.post('/webhook/retell-callback', async (req, res) => {
      ...
      const bookingLinkSent = meetingInterested === 'Yes' ? 'Yes' : 'No';
           ...
           // Use safeUpdateZohoLead to split free-text and picklist fields
           // and get detailed per-field error logging
